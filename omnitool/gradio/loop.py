...
def sampling_loop_sync(
    *,
    ...
    elif model in set(["omniparser + gemini-2.5-pro-exp-03-25", "omniparser + gemini-2.0-flash", "omniparser + gemini-2.0-flash-lite", "omniparser + gemini-2.5-flash-preview-04-17", "omniparser + gemini-1.5-pro", "omniparser + gpt-4o", "omniparser + o1", "omniparser + o3-mini", "omniparser + R1", "omniparser + qwen2.5vl"]):
        actor = VLMAgent(
            model=model,
            provider=provider,
            api_key=api_key,
    ...
    if model == "claude-3-5-sonnet-20241022": # Anthropic loop
        ...
        while True:
            ...
            if not tool_result_content:
                return messages
    elif model in set(["omniparser + gemini-2.5-pro-exp-03-25", "omniparser + gemini-2.0-flash", "omniparser + gemini-2.0-flash-lite", "omniparser + gemini-2.5-flash-preview-04-17", "omniparser + gemini-1.5-pro"]):
        while True:
            parsed_screen = omniparser_client()
            tools_use_needed, vlm_response_json = actor(messages=messages, parsed_screen=parsed_screen)
